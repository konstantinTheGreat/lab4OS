...
char *receive_message(const char *fifo_name, int buffer_size) {
	if(fifo_name == NULL) {
		printf("Attention: the answer is on server\n");
		return "";
	}
...
int main(int argc, char **argv) {
	...
	void stop_handler(int nsig) {
		send_message(write_fifo, "stop");
		printf("\nStop program\n");
    ...
    while (1) {
        ...
        char *server_output = receive_message(read_fifo, 1000);
        printf("Результат выполнения команды на сервере:\n%s\n", server_output);

		if(strncmp(message, "pid ", 4) == 0) {
			char *pos = strstr(message, " ");
			int signal = atoi(pos + 1);
			int pid = atoi(server_output);
			kill(pid, signal);
		}
